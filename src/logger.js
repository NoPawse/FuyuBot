...
function cmd(cmd, evt, suffix) {
  ...
  console.log(chalk.magenta.bold(`[FuyuBot]`), chalk.blue(`[${moment().format('DD-MM-YYYY HH:mm:ss')}]`), chalk.bold.green(`[${guildid}]`), chalk.bold.green(`[${evt.message.author.username}]`), chalk.green(`${nconf.get('PREFIX')}${cmd}`), suffix);
}

function info(msg) {
  if (production) return _submitToLogger('info', msg);
  console.log(chalk.magenta.bold(`[FuyuBot]`), chalk.blue(`[${moment().format('DD-MM-YYYY HH:mm:ss')}]`), chalk.cyan(`[INFO] ${msg}`));
}
...
function success(msg) {
  ...
  console.log(chalk.magenta.bold(`[FuyuBot]`), chalk.blue(`[${moment().format('DD-MM-YYYY HH:mm:ss')}]`), chalk.green(`${msg}`));
}

function warn(msg) {
  if (production) return _submitToLogger('warn', msg);
  console.log(chalk.magenta.bold(`[FuyuBot]`), chalk.blue(`[${moment().format('DD-MM-YYYY HH:mm:ss')}]`), chalk.yellow(`[WARN] ${msg}`));
}

function error(msg) {
  if (production) return _submitToLogger('error', msg);
  console.log(chalk.magenta.bold(`[FuyuBot]`), chalk.blue(`[${moment().format('DD-MM-YYYY HH:mm:ss')}]`), chalk.red(`[ERROR] ${msg} ++`));
}
